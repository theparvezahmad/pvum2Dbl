!***********************************************************************
!Development of a 2D Boundary layer compressible flow solver using PVUM+ Scheme
!Under the Guidance of Prof. Nadeem Hasan
!Group Members: Parvez Ahmad, Ayush Gupta, Aqib Khan
!***********************************************************************
program StartPVUM
implicit none
real(kind=8),parameter::timeStep=2.0d-05,Gamma=1.4d0,Mach=0.7d0,Pr=0.70d0,kay=0.002d0,kax=0.1d0,eps=1.0d-08,Re=100000.0d0
real(kind=8),allocatable,dimension(:)::      X,Y
real(kind=8),dimension(:,:),allocatable::    Rho,totEnrgy,Enrgy,Temp,Press,Mu,kappa,hT
real(kind=8),dimension(:,:,:),allocatable::  U,Vel,FcIntCel,GcIntCel,Fnc,Gnc,Q,U1,JS
!Temporary Variables
...
real(kind=8)::                               percent,time,Yb,p1,p2,p3,uu1,uu2,uu3,dudy1,dTdy1,shear,heat_flux,cc1,cc2,cc3
...
allocate(FcIntCel(nX-1,2:nY-1,4),GcIntCel(2:nX-1,nY-1,4),Q(nX,nY,4),JS(nX,nY,4))
...
U1(i,j,k)=U(i,j,k)
U(i,j,k)=U(i,j,k)-timeStep*(tmp1+tmp2-JS(i,j,k))
end do!loop k ends
end do!loop j ends
end do!loop i ends
!*****************Predictor Step ends***********************************
...
U(i,j,k)=(U1(i,j,k)+U(i,j,k)-timeStep*(tmp1+tmp2-JS(i,j,k)))/2.0
end do!loop k ends
end do!loop j ends
end do!loop i ends
call findPrim()
...
open(17,file='profile_x_0_3.dat',status='unknown')
i=1
do while (x(i) < 0.3)
i=i+1
end do  
...
do j=1,nyb
write(17,*)y(j),rho(i,j),vel(i,j,1),vel(i,j,2),temp(i,j),press(i,j)
end do

close(17)
    
open(17,file='profile_x_0_6.dat',status='unknown')
i=1
do while (x(i) < 0.6)
i=i+1
end do  
...
open(17,file='profile_x_0_9.dat',status='unknown')
i=1
do while (x(i) < 0.9)
i=i+1
end do  
...
if(abs(u3-u2)+abs(u2-u1).gt.eps)then
Z(1)=(abs(u3-u2)-abs(u2-u1))/(abs(u3-u2)+abs(u2-u1)) 
else 
Z(1)=0.3
end if

if(abs(u4-u3)+abs(u3-u2).gt.eps)then
...
phiWf=phiC+Wf*(phiU-phiC)
  ...
  if(abs(uu3-uu2)+abs(uu2-uu1).gt.eps)then 
  Z(1)=(abs(uu3-uu2)-abs(uu2-uu1))/(abs(uu3-uu2)+abs(uu2-uu1));
  else 
  Z(1)=0.3
  end if

  if(abs(uu4-uu3)+abs(uu3-uu2).gt.eps)then 
...
Fnc(i,j,1)=0.0
Fnc(i,j,2)=Press(i,j)-2.0*Mu(i,j)*(dudx-divVel/3.0)/Re
Fnc(i,j,3)=-Mu(i,j)/Re*(dvdx+dudy)
Fnc(i,j,4)=-Gamma*kappa(i,j)*dTdx/(Re*Pr)+Gamma*(Gamma-1)*Mach*Mach*(Mu(i,j)*Df/Re+vel(i,j,1)*press(i,j))

Gnc(i,j,1)=0.0
Gnc(i,j,2)=-Mu(i,j)*(dvdx+dudy)/Re
Gnc(i,j,3)=Press(i,j)-2.0*Mu(i,j)*(dvdy-divVel/3.0)/Re
Gnc(i,j,4)=-Gamma*kappa(i,j)*dTdy/(Re*Pr)+Gamma*(Gamma-1)*Mach*Mach*(Mu(i,j)*Dg/Re+vel(i,j,2)*press(i,j))

JS(i,j,4)=-(Gamma-1)*divVel
end do
end do
...
kappa(i,j)=Mu(i,j)
Press(i,j)=(Rho(i,j)*Temp(i,j)-1)/(Gamma*Mach*Mach)
Enrgy(i,j)=Temp(i,j)
totEnrgy(i,j)=Temp(i,j)+0.5*Gamma*(Gamma-1)*Mach**2*(Vel(i,j,1)**2+Vel(i,j,2)**2)
hT(i,j)=totEnrgy(i,j)+(Gamma-1)*Temp(i,j)
end do
end do
...
implicit none
real(kind=8)::dpdy1,dpdy2,dpdy3,R_pos,R_neg,t2,t3,Entpy,p1,p2,p3,dtxy,rru,rrv,qq,divvel,dtyy,dpdy,dudyb
...
dpdy=((dtxy+dtyy)/Re-Rho(i,2)*(vel(i,2,2)**2)/(Y(2)-Y(1)))
...
Rho(i,1)=(1+Gamma*Mach*Mach*press(i,1))/Temp(i,1)
end do
...
hT(i,1)=totEnrgy(i,1)+(Gamma-1)*Temp(i,1)
end do
...
do j=1,nYb
Press(1,j)=(Temp(1,j)*Rho(1,j)-1)/(Gamma*Mach*Mach)
...
hT(1,j)=totEnrgy(1,j)+(Gamma-1)*Temp(1,j)
end do
...
if(Vel(i,nYb,2)<=0.0) then !inflow
  ...
  if(abs(Vel(i,nYb,2)*Mach/sqrt(Temp(i,nYb)))>1.0) then
  ...
  Rho(i,nYb)=Temp(i,nYb)**(1/(Gamma-1))
else
  t2=Vel(i,nYb-1,2)+2.0*sqrt(Temp(i,nYb-1))/(Mach*(gamma-1.0))
  t3=Vel(i,nYb-2,2)+2.0*sqrt(Temp(i,nYb-2))/(Mach*(gamma-1.0))
  R_pos=t3-(t3-t2)/(Y(nYb-2)-Y(nYb-1))*(Y(nYb-2)-Y(nYb))

  if(abs(Vel(1,nYb,2)*Mach/sqrt(Temp(1,nYb)))>1.0) then
  t2=Vel(i,nYb-1,2)-2.0*sqrt(Temp(i,nYb-1))/(Mach*(gamma-1.0))
  t3=Vel(i,nYb-2,2)-2.0*sqrt(Temp(i,nYb-2))/(Mach*(gamma-1.0))
...
do i=1,nX
Press(i,nYb)=(Temp(i,nYb)*Rho(i,nYb)-1)/(Gamma*Mach*Mach)
...
hT(i,nYb)=totEnrgy(i,nYb)+(Gamma-1)*Temp(i,nYb)
end do
...
if(Vel(nX,j,1)<0.0) then !inflow
  ...
  R_neg=Vel(nX,j,1)-2.0*sqrt(Temp(nX,j))/(Mach*(gamma-1.0))
  end if
 ...
 Rho(nX,j)=Rho(nX-2,j)+((Rho(nX-1,j)-Rho(nX-2,j))/(X(nX-1)-X(nX-2)))*(X(nX)-X(nX-2))
...
do j=1,nYb
Press(nX,j)=(Temp(nX,j)*Rho(nX,j)-1)/(Gamma*Mach*Mach)
...
hT(nX,j)=totEnrgy(nX,j)+(Gamma-1)*Temp(nX,j)
end do
